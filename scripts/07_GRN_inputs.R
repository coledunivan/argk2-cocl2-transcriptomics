...
# (~6 MB vs ~79 MB plain). readr reads .gz transparently, so we just need to
# hand it whichever copy exists locally.
.resolve_tflink <- function() {
...
TFLINK_FILE       <- .resolve_tflink()   # CSV/TSV, plain or gzipped
...
if (length(orphans) > 0) {
  ...
  extra_tfs <- tflink |>
    dplyr::filter(
      target %in% orphan_tflink_names,
      !(tf %in% CHARACTERISTIC_TFS)
    ) |>
    dplyr::count(
      tf,
      name = "n_orphans_covered"
    ) |>
    dplyr::arrange(
      dplyr::desc(n_orphans_covered)
    ) |>
    dplyr::mutate(
      pct_orphans = paste0(
        round(
          100 * n_orphans_covered / length(orphans)
        ),
        "%"
      )
    )
