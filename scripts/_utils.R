...
#   contains the 6 MAH205/argk-1 columns that were excluded from analysis,
...
# ---- TFLink / count-matrix resolution ---------------------------------------
# The merged TFLink x count matrix ships gzipped in the GitHub repo
# (data/reference/RNAseq_to_TF_Targets.csv.gz, ~6 MB) because the plain CSV is
# ~79 MB. R reads .gz transparently, so callers just need the right filename.
resolve_tflink_path <- function() {
  for (p in c("data/reference/RNAseq_to_TF_Targets.csv",
              "data/reference/RNAseq_to_TF_Targets.csv.gz")) {
    if (file.exists(p)) return(p)
  }
  stop("Could not find data/reference/RNAseq_to_TF_Targets.csv[.gz] - ",
