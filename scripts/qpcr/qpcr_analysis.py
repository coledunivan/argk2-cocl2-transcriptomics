...
QPCR_ROOT = ROOT / "data" / "qPCR" / "runs"                 # raw Cq plates
QPCR_EFF  = ROOT / "data" / "qPCR" / "efficiency_curves"    # standard curves
QPCR_OUT  = ROOT / "outputs" / "qPCR"                       # tables
SUPP_FIG  = ROOT / "outputs" / "supplementary"  # figure panels
...
def compute_delta_cq(df, ref_gene=REF_GENE, efficiencies=None):
    ...
    If `efficiencies` is supplied (a dict mapping target name -> amplification
    fold per cycle, where 2.0 = 100% efficient), Pfaffl-corrected values are
    computed instead:
        eff_dcq = Cq_target * log2(E_target) - mean(Cq_ref) * log2(E_ref)
    When E_target = E_ref = 2, this collapses to the classic formula.
    Downstream ddcq -> log2fc math is unchanged; the correction enters
    linearly through the per-well eff_dcq. See Pfaffl MW (2001), NAR 29:e45.

    Returns long-format DataFrame: run, target, strain, treatment, dcq.
    """
    df = df[df["keep"]].copy()
    # Resolve per-target and per-ref efficiency multipliers (log2 of fold)
    if efficiencies is not None:
        log2_E_ref = np.log2(efficiencies.get(ref_gene, 2.0))
    else:
        log2_E_ref = 1.0  # classic Livak: assumes E = 2
    out = []
    for (run, strain, treatment), grp in df.groupby(["run","strain","treatment"]):
        ...
        for _, row in grp.iterrows():
            if row["target"] == ref_gene:
                continue
            if efficiencies is not None:
                log2_E_t = np.log2(efficiencies.get(row["target"], 2.0))
            else:
                log2_E_t = 1.0
            dcq = row["cq"] * log2_E_t - ref_mean * log2_E_ref
            out.append({
                "run":       run,
                "target":    row["target"],
                "strain":    strain,
...
COMPARISON_CSV = str(QPCR_OUT / "qPCR_vs_RNAseq_comparison.csv")
...
def _load_comparison():
    ...
    try:
        ...
        print(f"[warn] could not read {COMPARISON_CSV}: {e}")
        return None
    if df.empty:
        return None
...
def _draw_panel_c(ax, comp_df):
    ...
    ax.set_ylabel(r"qPCR log$_2$(fold change)"
                  + "\n"
                  + r"(efficiency-corrected $\Delta\Delta$C$_q$, norm. to $\it{tba\text{-}1}$)",
                  fontsize=9.5)
...
def make_figure(agg_df, ddcq_df, out_pdf, out_png, subtitle=None):
    ...
    ax_bar.set_xticklabels(gene_order, rotation=40, ha="right", fontsize=10,
                           style="italic")
    ax_bar.set_ylabel(r"log$_2$(fold change)" + "\n"
                      + r"[qPCR, efficiency-corrected $\Delta\Delta$C$_q$,"
                      + "\n"
                      + r"normalized to $\it{tba\text{-}1}$]",
                      fontsize=10)
...
def main():
    ...
    agg_path  = QPCR_OUT / "qPCR_combined_deltadeltaCq.csv"
    ddcq_path = QPCR_OUT / "qPCR_per_run_ddcq.csv"

    if agg_path.exists() and ddcq_path.exists():
        print("[step 1-5] Pre-computed results found — skipping raw-data pipeline.")
        ...
        ddcq_save.to_csv(ddcq_path, index=False)
        print(f"  Wrote {ddcq_path}")
        agg_df.to_csv(agg_path, index=False)
        print(f"  Wrote {agg_path} (all rows)")
    ...
    if comp_all is not None:
        make_supplement_figure(comp_all,
                               SUPP_FIG / "qPCR_supplement_pergene.pdf",
                               SUPP_FIG / "qPCR_supplement_pergene.png")
    else:
